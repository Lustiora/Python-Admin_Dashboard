...
def view_table_rental_data(rental_data, rental_id_data, connect_module, connect_module_count, connect_module_page:int, query, page_num):
    if rental_id_data:
...
def build_rental_ui(page, store_id, conn):
    ...
    def rental_search_total_query(e, view_page):
        ...
        try:
            ...
            view_table_rental_data(rental_data, rental_id_data, connect_module, connect_module_count,
                                   connect_module_page, total_rental_data, page_num)
        except Exception as err:
    ...
    def rental_search_overdue_query(e, view_page):
        ...
        try:
            ...
            rental_id_data = cursor.fetchall()
            view_table_rental_data(rental_data, rental_id_data, connect_module, connect_module_count,
                                   connect_module_page, overdue_data, page_num)
        except Exception as err:
            print(f"Search Rental error : {err}")
    ...
    def rental_search_due_today_query(e, view_page):
        ...
        try:
            ...
            rental_id_data = cursor.fetchall()
            view_table_rental_data(rental_data, rental_id_data, connect_module, connect_module_count,
                                   connect_module_page, due_today_data, page_num)
        except Exception as err:
            print(f"Search Rental error : {err}")
    ...
    def rental_search_data_query(e, view_page):
        ...
        try:
            ...
            rental_id_data = cursor.fetchall()
            view_table_rental_data(rental_data, rental_id_data, connect_module, connect_module_count,
                                   connect_module_page, connect_module_count[0], page_num)
        except Exception as err:
            print(f"Search Rental error : {err}")

    input_rental = flet.TextField(
        hint_text=" Press Enter to Search", on_submit=lambda e:rental_search_data_query(None, view_page),
